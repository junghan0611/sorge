...
3. **파생 가능한 것은 저장하지 않는다. 그러나 판정은 저장한다.** 커밋 수·도장 날짜·스킬
   ...
   「있냐 없냐」 씨름이 된다. 판정은 `배정`(+id) / `관리 안 함` / `보류` / `불필요` **넷**이다 (`LEDGER.md § 읽는 법`).
4. **순회의 출력은 여전히 브리핑까지다. 그러나 「부르는 손은 GLG 하나」는 폐기됐다.**
   ~~자동 발신 금지 — 형제를 부르는 것은 GLG의 손이다~~ 였고, **GLG 가 2026-09-07 에 뒤집었다:**
   *"내 손을 떠나야 한다."*
   **가르는 선:** 워크트리를 파고 그 집 시민을 앉히는 것은 **루프**의 절차이지(`LOOP.md § 착수`)
   이 스킬이 하는 일이 아니다 — 이 스킬은 `--brief` 로 블록을 만들어 던진다.
   두 경우 모두 **앉는 자는 언제나 그 집의 시민이고, 메인에 미는 손은 언제나 GLG** 다.
   남의 리포에 직접 커밋하는 것은 여전히 금지다.
