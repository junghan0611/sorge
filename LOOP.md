...
**손이 둘이고 권한이 다르다.** 이 문서의 `scope`·`never` 는 **첫째 손**의 것이다.

| 손 | 무엇 | 권한 |
|---|---|---|
| **순회 sorge** — 사람이 부를 때 도는 이 문서의 주체 | 발견·명명·전달, 라벨 판정 | **라벨만.** 코멘트·닫기·남의 리포 수정 전부 금지 |
| **stock 루프 `sorge-bot`** — GLG 가 2026-09-15 에 켠 RobOMP | 이슈 사건마다 fresh 턴 | stock 흐름 그대로 — **분류·댓글·브랜치·PR 까지.** 우리가 좁히지 않는다 |

둘째 손을 우리가 설계하지 않는 것이 그 판정의 요점이다 — *"omp 를 수정하면 안돼.
그래야 omp 버전업을 할수가 있거든."* 좁히고 싶어지면 fork 가 아니라 **프롬프트·라벨
규약·이슈 본문**으로 좁힌다. 운전법은 담당자 문서 `denote:20260227T031800` §절차.

**양쪽 다 닫혀 있는 것은 하나뿐이다: 무인 close.**
`ROBOMP_QUESTION_AUTOCLOSE_ENABLED=false` — stock 기본값이 `true` 인데 뒤집었다.
무인으로 GLG 의 이슈를 닫는 것은 기본값이어도 받지 않는다(이것이 「stock 그대로」의
유일한 예외이고, 예외임을 여기 적어 둔다).
...
## never — 첫째 손(순회 sorge)에 대해

이 목록은 **순회 sorge** 를 묶는다. stock 루프의 `sorge-bot` 은 GLG 가 켠 별개의 손이고
§mission 표가 그 권한을 든다 — 여기 금지된 코멘트를 그 봇은 쓴다. 섞어 읽지 마라.
