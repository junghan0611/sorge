...
- [x] **8. 이슈판 기반** — forge DB read-only 허가·pull 리듬(`my/forge-pull-all`)·동시성 확인, Datasette 1층 설치(`nixos-config` `ce7e3f8`). Forgejo는 REST 별축 (2026-09-04)
- [ ] **9. forge + Datasette** ← **다음 세션 (GLG 지정)**: 이슈판을 실제로 세운다. 오늘은 여기까지 — 기반만 놓고 닫았다
...
- **Next — GLG 지정: `forge` + Datasette 를 이어간다.**
  ...
  - **대장은 이슈 상태를 들지 않는다** — 전부 유도 가능하다. 들 것은 「이 이슈 묶음은 한 덩어리다」라는
    사람의 판정 하나뿐이다.
- **빚은 급하지 않다 (GLG 판정 2026-09-04).** *"일단 좀 debt 있어도 된다. 가든에 담당자 문서는 가끔
  업데이트해도 좋아. 내가 오늘 말한것은 **담당자 문서가 없다는게 문제**라고 하는거야. 업데이트는
  대화하다가 보면 해야겠다 싶을때가 있거든 tag-release 할때라든가."*
  **그러니 순회의 무게는 「빚」이 아니라 「문서 미정」에 있다.** 빚 숫자를 몰아붙이지 않는다.
- **이 방의 제목을 오늘 처음 내가 지었다.** GLG가 `§sorge #담당자` 표시만 남기고 뒷자리를 비워 뒀던
  자리다 — `대신 해주지 않고 앞서 간다 — 판정만 드는 대장과 계를 가로지르는 발견`.
  앞쪽은 Fürsorge 두 갈래 중 이 집이 고른 쪽, 뒤쪽은 그것이 실제로 무엇으로 만들어져 있는지다.
- **`sweep.py` 가 병렬 파서를 버렸다 (`6f02e14`).** `denotecli` 가 `hugo_lastmod`·abstract 를 싣게
  되면서(`3c57689`) 사본이 존재할 이유가 사라졌다. 바꾸기 전 8/8 도장 일치 확인.
