...
junghan0611/sorge#12	house=nixos-config(배정 — GLG 2026-09-08 대장 등재: nixos-config=20260615T100659) verdict=재분류필요(대장 등재로 대상 진입 — 이전 「보류(대상 아님)」은 낡았다) state=미분류
...
junghan0611/entwurf#78	house=entwurf(배정, 20260302T191200) kind=기능 authority=쓰기 reversible=예(쓰기가 브랜치 안) money=아니오 trust=GLG본인 deadline=없음 ball=담당자 verdict=담당자몫 state=착수 branch=fix/78-realhome-flavor@d67aff9 coord=20260908T211235-485de8@claude-code/fable impl=20260908T211458-5bbccd@claude-code/opus done_when="코디네이터 레인 — 포커스 게이트 → 다른 학교 검수 → amendment → qualification 1회 + check:full 1회 → 브랜치 커밋" 곁=**이 브랜치는 `#78` 의 한 칸(First evidence)만 닫는다. `#78` 자체는 안 닫힌다** — 나머지 다섯 칸은 물리 호스트를 요구한다. **하위 브랜치 산출물과 이슈 생애 state 를 섞지 않는다**(terra 검수 ⑥, 2026-09-08). 1차 워크트리 attempt(종료·소스 0줄·wt 제거됨)의 receipt 전량은 tick log 세 블록에 있다
...
junghan0611/sorge#6	house=agent-config·entwurf(배정 — GLG 2026-09-08 대장 등재: entwurf=20260302T191200 · agent-config=20260312T174622) verdict=재분류필요(대장 등재로 대상 진입 — 이전 「보류(대상 아님)」은 낡았다) state=미분류
junghan0611/sorge#7	house=agent-config(배정 — GLG 2026-09-08 대장 등재: agent-config=20260312T174622) verdict=재분류필요(대장 등재로 대상 진입 — 이전 「보류(대상 아님)」은 낡았다) state=미분류
